...
const ProjectDetails = ({ params }) => {
  ...
  const singleData = projects?.data?.find(
              ...
              <div className="relative">
                ...
                <div className="absolute top-2/3 left-0 right-0 text-center">
                  ...
                  <p className="text-md pt-3 font-semibold text-zinc-700 dark:text-zinc-300 flex items-center justify-center ">
                    {singleData?.tag?.map((item, index) => (
                      <React.Fragment key={item}>
                        <span>{item}</span>
                        {index !== singleData?.tag?.length - 1 && (
                ...
                <div className="flex lg:flex-row md:flex-row flex-col lg:items-start md:items-start items-center lg:justify-between md:justify-between justify-center gap-x-5 w-[100%] lg:gap-5 md:gap-5 gap-10">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex-wrap flex gap-3 mt-5">
                      {singleData?.technologies?.map((item) => (
                        <h1
                          key={Math.random()}
